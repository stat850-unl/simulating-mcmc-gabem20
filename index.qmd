...
where $R$ is the end-to-end distance, and the mean square position of the units is $\langle r^2\rangle = N u^2$ where $N$ is the number of units in the chain and $u$ is the length of each unit. 
...
return_n_dist <- function(n){ #, seed = 1202
  ...
  chain_dist <- sqrt(sum(x)^2+sum(y)^2+sum(z)^2)
  # print(origin_dist)
  # chain_dist <- sum(origin_dist)
...
```{r}
set.seed(1202)
uncon_sim_1000 <- lapply(rep(10, 1000), return_n_dist) %>% unlist()
```
...
constrained_chain <- function(n){
  ...
  vectors <- matrix(c(x, y, z), ncol = 1)

  
    while (ncol(vectors) < n){
    ...
    last_vector <- vectors[, ncol(vectors)]
    
    if ((pi - acos(sum(last_vector * new_vector))) >= pi/4){
      vectors <- cbind(vectors, new_vector)
    }
  }
  
  poly_length <- sqrt(sum(rowSums(vectors)^2))
  
  return(poly_length)
  
}

# constrained_chain(10)
...
I wrote the function above but not sure if my way of determine the angle between the vectors was correct. I found the angle between the vectors then subtracted this from pi given they are touching end to end, this *should* be the smallest angle.

I used vectors and matrices to store the objects and when it came to finding the final length of the polymer I summed across the rows, squared the values, summed these, then square rooted to get the final length.
...
```{r}
set.seed(1202)
con_sim_1000 <- lapply(rep(10, 1000), constrained_chain) %>%  unlist()


ggplot() +
  geom_density(aes(x = uncon_sim_1000,fill = "Unconstrained"), , alpha = 0.5) +
  geom_density(aes(x = con_sim_1000, fill = "Constrained"), alpha = 0.5) +
  scale_color_manual(values = c("Unconstrained" = "blue",
                                "Constrained" = "red")) +
  labs(
    title = "Comparison of Polymer Length Densities When Constrained and Unconstrained",
    x = "Length",
    y = "Density",
    fill = "Data"
  )
